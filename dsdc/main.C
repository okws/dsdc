...
usage (bool err = true)
{
    ...
    warnx << "usage: " << progname << " -M [-d<debug-level>] "
          ...
          << "  -X proxy node:\n"
          << "\n"
          << "     Make this DSDC node run as a proxy node, meaning that it\n"
          << "     will be proxying requests to actual DSDC slaves. Supply\n"
          << "     the names of the masters to connect to as arguments,\n"
          << "     in <host>:<port> format.\n"
          << "\n"
          << "    Sub-options:\n"
          << "\n"
